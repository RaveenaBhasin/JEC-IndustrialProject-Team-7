...
//asuming these ports are available
std::string ports[] = {"3000", "3001", "3002", "3003", "3004", "3005", "3006", "3007", "3008", "3009"};
...
class Client {
    ...
    int loginClient() {
        ...
        tracker.close();    
        send(this->socket, "Credentials do not match! Try again", 36, 0);
        return this->loginClient();
    }
    void welcome() {
...
int authenticateClient(Client* client) {
    ...
    while(true) {
        ...
        if(strcmp(msg, "Register") == 0 || strcmp(msg, "register") == 0) {
            ...
            //login client
            send(clientSocket, "Login format(uniqueID:Password)", 32, 0);
...
void *Messaging(void* arg) {
    ...
    int clientSocket = client->socket;
    char msg[4096] = {0};
    send(clientSocket, client->servPort.c_str(), client->servPort.size(), 0);
    ...
    while(true) {
        ...
        if(bytesRecv == 0) {
            std::cout << "The client " << clientID << " disconnected\n";
            break;
        }
        ...
        } else if(res[0].compare("connect")==0) {
            if(res.size()!=2) {
                send(clientSocket, "Invalid!", 9, 0);
                continue;
            }
            std::string servPort = onlinePeers[res[1]]->servPort;
            send(clientSocket, servPort.c_str(), servPort.size(), 0);
        } else {
